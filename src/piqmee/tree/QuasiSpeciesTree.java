...
public class QuasiSpeciesTree extends Tree {
    ...
    protected QuasiSpeciesIncidence[] incidences;
...
//            }
    ...
    */
    ...
    public void initFromUniqueHaploTree(Tree uniqueHaploTree, Alignment data, boolean collapseIdentical,
        ...
        List<String> incidenceTaxa = storeIncidenceData(data, uniqueHaploTree);

        ArrayList result = processNextNodeOfFullNewickTree(
                uniqueHaploTree.getRoot(), qsTips, qsInternalNodes,
                distanceMatrix, haplotypesSeen, incidenceTaxa, collapseIdentical);
    ...
    public void initFromFullTree(Tree fullTree, Alignment data, boolean collapseIdentical,
        ...
        List<String> incidenceTaxa = storeIncidenceData(data, fullTree);
    ...
    private ArrayList processNextNodeOfFullNewickTree(
            ...
            double[][] distanceMatrix, ArrayList haplotypesSeen, List<String> incidenceTaxa, boolean collapseIdentical) {
     ...
     *
     * @param data
     * @return A list of all incidence taxa.
     */
    protected List<String> storeIncidenceData(Alignment data, Tree tree) {
        Map<String, Integer> nodes = new HashMap<>();
        for (Node node : tree.getExternalNodes())
            nodes.put(node.getID(), node.getNr());

        Map<Double, QuasiSpeciesIncidence> incidenceMap = new HashMap<>();

        List<String> incidenceTaxa = new ArrayList<>();
        ...
        for (Sequence sequence : data.sequenceInput.get()) {
            ...
            if (p.matcher(sequence.getData()).matches()) {
                ...
                int nodeId = nodes.get(taxon);
                Node node = tree.getNode(nodeId);
                double samplingTime = node.getHeight();
                double attachmentTime = node.getParent().getHeight();
                int count = haplotypeCounts.get(taxon);

                if (!incidenceMap.containsKey(samplingTime)) {
                    // store an incidence for each unique sampling time
                    QuasiSpeciesIncidence newIncidence = new QuasiSpeciesIncidence(samplingTime, count);
                    newIncidence.addAttachmentTime(attachmentTime);
                    incidenceMap.put(samplingTime, newIncidence);
                } else {
                    // sum up counts of sequences with equal times
                    QuasiSpeciesIncidence incidence = incidenceMap.get(samplingTime);
                    incidence.addCount(count);
                    incidence.addAttachmentTime(attachmentTime);
                }
            }
        }

        // generate missing attachment times
        for (QuasiSpeciesIncidence incidence : incidenceMap.values()) {
            incidence.generateAttachmentTimes(tree.getRoot().getHeight());
        }

        incidences = incidenceMap.values().toArray(new QuasiSpeciesIncidence[0]);
     ...
     */
    public QuasiSpeciesIncidence[] getIncidences() {
        return incidences;
    }
